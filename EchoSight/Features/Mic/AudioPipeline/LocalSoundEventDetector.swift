// MARK: - File Guide
// A lightweight local sound classifier. It uses audio energy patterns to
// label rough sound events without needing a server.
...
final class SoundEventService {
    ...
    private func countPeaks(values: [Float], above: Float) -> Int {
        ...
        var peaks = 0
        for valueIndex in 1..<(values.count - 1) {
            let previousValue = values[valueIndex - 1]
            let currentValue = values[valueIndex]
            let nextValue = values[valueIndex + 1]
            if currentValue > previousValue && currentValue > nextValue && currentValue > above {
                peaks += 1
            }
        }
        return peaks
